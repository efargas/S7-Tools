...
#### Simulated Testing Environment (Phase 6)

**Problem**: Testing the complete 7-stage bootloader workflow requires access to a physical S7-1200 PLC, modbus power supply, and serial adapter, which causes friction for rapid development.
**Solution**: A fully simulated environment utilizing `socat` for virtual serial ports and a lightweight Python script (`simulated_plc.py`) that implements the PLC-side bootloader state machine natively.

**Architecture**:
- **Virtual Serial Device** created via `socat pty,raw,echo=0,link=/tmp/ttyV0 pty,raw,echo=0,link=/tmp/ttyV1`
- **PLC Simulator** listens on `/tmp/ttyV1` and responds to the S7Tools application over `/tmp/ttyV0`
- Provides realistic latency, handshake negotiations, and dummy payloads mimicking the native firmware

**Benefits**:
- Run end-to-end memory dump testing without hardware
- Validate UI progress reporting logic deterministically
- Reproduce complex race conditions safely
