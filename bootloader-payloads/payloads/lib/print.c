/**
 * @file print.c
 * @brief UART communication functions for Siemens S7 PLC payloads
 * 
 * This file implements UART protocol functions for sending data from
 * ARM payloads running on the PLC back to the host computer. It includes
 * both single character output and packet-based protocol communication.
 */
...
/**
 * @brief Output a single character via UART
 * @param c Character to output
 * @return The character that was output
 * 
 * Waits for UART transmit buffer to be ready, then sends the character.
 * Based on gdbstub implementation from esp-gdbstub project.
 */
int putchar(int c) {
    while (*uart2_rh & 0x20) {
...
#ifdef NEW_UART_SEND
/**
 * @brief Send a single packet using the UART protocol
 * @param s Data buffer to send
 * @param size Number of bytes to send (must be < 255)
 * @return Number of bytes sent, or -1 on error
 * 
 * Sends data in the bootloader's packet format:
 * <length_byte><data_bytes><checksum_byte>
...
/**
 * @brief Send large amounts of data by chunking into multiple packets
 * @param s Data buffer to send
 * @param len Total number of bytes to send
 * @return Total number of bytes sent
 * 
 * Splits large data into 32-byte chunks and sends each as a separate packet.
 * Sends an empty packet at the end to signal completion.
 */
