...
/// <summary>
/// Defines the contract for managing serial port profiles including CRUD operations, import/export, and validation.
/// This service provides comprehensive profile management capabilities for the S7Tools application.
/// </summary>
public interface ISerialPortProfileService
{
    #region Profile CRUD Operations

    /// <summary>
    /// Retrieves all available serial port profiles.
    /// </summary>
    ...
    Task<IEnumerable<SerialPortProfile>> GetAllProfilesAsync(CancellationToken cancellationToken = default);
    ...
    Task<SerialPortProfile?> GetProfileByIdAsync(int profileId, CancellationToken cancellationToken = default);
    ...
    Task<SerialPortProfile?> GetProfileByNameAsync(string profileName, CancellationToken cancellationToken = default);

    /// <summary>
    /// Creates a new serial port profile.
    /// </summary>
    ...
    Task<SerialPortProfile> CreateProfileAsync(SerialPortProfile profile, CancellationToken cancellationToken = default);

    /// <summary>
    /// Updates an existing serial port profile.
    /// </summary>
    ...
    Task<SerialPortProfile> UpdateProfileAsync(SerialPortProfile profile, CancellationToken cancellationToken = default);

    /// <summary>
    /// Deletes a serial port profile by its unique identifier.
    /// </summary>
    ...
    Task<SerialPortProfile> DuplicateProfileAsync(int sourceProfileId, string newName, CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets the default profile for new connections.
    /// </summary>
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    ...
    Task<SerialPortProfile> GetDefaultProfileAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Sets a profile as the default profile for new connections.
    /// </summary>
    ...
    Task<SerialPortProfile> EnsureDefaultProfileExistsAsync(CancellationToken cancellationToken = default);
    ...
    Task<SerialPortProfile> ImportProfileFromJsonAsync(string jsonData, bool overwriteExisting = false, CancellationToken cancellationToken = default);
    ...
    Task<IEnumerable<SerialPortProfile>> ImportProfilesFromJsonAsync(string jsonData, bool overwriteExisting = false, CancellationToken cancellationToken = default);
    ...
    /// <exception cref="ArgumentNullException">Thrown when profile is null.</exception>
    IEnumerable<string> ValidateProfile(SerialPortProfile profile);
