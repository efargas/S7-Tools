...
{
    /// <summary>
    /// Represents the PlcProtocolHandler.
    /// </summary>
    internal class PlcProtocolHandler(IPlcProtocol protocol)
    {
        ...
        {
            await _protocol.DisconnectAsync(cancellationToken).ConfigureAwait(false);
        }
        ...
        /// <summary>
        /// Executes the InvokePrimaryHandlerAsync operation.
        /// </summary>
        ...
        /// <summary>
        /// Executes the InvokeAddHookAsync operation.
        /// </summary>
        ...
        {
            await _protocol.SendPacketAsync(payload, maxChunk, cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            await _protocol.RawWriteAsync(buffer, offset, count, cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            return await _protocol.RawReadAsync(buffer, offset, count, cancellationToken).ConfigureAwait(false);
        }
        ...
        /// <summary>
        /// Executes the PerformHandshakeAsync operation.
        /// </summary>
            ...
            for (int attempt = 0; attempt < MaxHandshakeAttempts; attempt++)
            {
                ...
                while (sw.ElapsedMilliseconds < 300)
                {
                    if (_protocol.DataAvailable)
                    {
                        ...
                        int bytesRead = await _protocol.RawReadAsync(tmpBuf, 0, tmpBuf.Length, cancellationToken).ConfigureAwait(false);
                        if (bytesRead > 0)
                        {
        ...
        /// <summary>
        /// Executes the GetVersionAsync operation.
        /// </summary>
        public async Task<byte[]> GetVersionAsync(CancellationToken cancellationToken)
        {
            // Handler 0x00 = Get Info
            byte[]? response = await InvokePrimaryHandlerAsync(0x00, [], true, cancellationToken).ConfigureAwait(false);
            return response ?? [];
        }
    }
}
