...
public class HexEditor : TemplatedControl
{
    ...
    /// </summary>
    public static readonly DirectProperty<HexEditor, int?> BytesPerLineProperty =
        AvaloniaProperty.RegisterDirect<HexEditor, int?>(
            nameof(BytesPerLine),
            editor => editor.BytesPerLine,
            (editor, value) => editor.BytesPerLine = value
        );

    /// <summary>
    /// Gets or sets the fixed amount of bytes per line that should be displayed.
    /// </summary>
    public int? BytesPerLine
    {
        get => HexView.BytesPerLine;
        set => HexView.BytesPerLine = value;
    }
