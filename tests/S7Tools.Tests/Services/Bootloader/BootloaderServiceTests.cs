...
public class EnhancedBootloaderServiceTests
{
    ...
    private static JobProfileSet CreateTestProfiles(int socatPort = 8080, string serialDevice = "/dev/ttyUSB0")
    {
        ...
        return new JobProfileSet(
            ...
            Power: new PowerProfileRef("192.168.1.100", 502, 0, 2, powerConfig, 1, ModbusAddressingMode.Base0),
        ...
        IResourceCoordinator? resourceCoordinator = null)
    {
        return new EnhancedBootloaderService(
            Microsoft.Extensions.Logging.Abstractions.NullLogger<EnhancedBootloaderService>.Instance,
    ...
    public async Task DumpAsync_WithValidConfiguration_ShouldExecute7StageWorkflow()
    {
        ...
        // Act
        BootloaderResult result = await service.DumpAsync(profiles, progress, null, null, CancellationToken.None);

        // Assert
        result.Should().NotBeNull();
        result.SavedFiles.Count.Should().Be(1); // Expected dump size

        // Verify all stages executed
        await plcClient.Received(1).InvokeDumperStreamAsync(
            Arg.Any<uint>(), Arg.Any<uint>(), Arg.Any<Func<ReadOnlyMemory<byte>, ValueTask>>(), Arg.Any<IProgress<long>>(), Arg.Any<CancellationToken>(), Arg.Any<bool>(), Arg.Any<Microsoft.Extensions.Logging.ILogger?>());
        ...
        await power.Received().ConnectAsync(Arg.Any<PowerSupplyConfiguration>(), Arg.Any<Microsoft.Extensions.Logging.ILogger?>(), Arg.Any<CancellationToken>());
        await power.Received().TurnOffAsync(Arg.Any<Microsoft.Extensions.Logging.ILogger?>(), Arg.Any<CancellationToken>());
    ...
    public async Task DumpAsync_ShouldReportProgressForEachStage()
    {
        ...
        IPowerSupplyService power = Substitute.For<IPowerSupplyService>();
        power.ConnectAsync(Arg.Any<PowerSupplyConfiguration>(), Arg.Any<Microsoft.Extensions.Logging.ILogger?>(), Arg.Any<CancellationToken>()).Returns(true);
        ...
        // Act
        await service.DumpAsync(profiles, progress, null, null, CancellationToken.None);
    ...
    public async Task DumpAsync_WithNullProfiles_ShouldThrowArgumentNullException()
    {
        ...
        // Act
        Func<Task> act = async () => await service.DumpAsync(null!, progress, null, null, CancellationToken.None);
    ...
    public async Task DumpAsync_WithNullProgress_ShouldThrowArgumentNullException()
    {
        ...
        // Act
        Func<Task> act = async () => await service.DumpAsync(profiles, null!, null, null, CancellationToken.None);
    ...
    public async Task DumpAsync_WhenPowerSupplyConnectionFails_ShouldThrowException()
    {
        ...
        // Act
        Func<Task> act = async () => await service.DumpAsync(profiles, progress, null, null, CancellationToken.None);
    ...
    public async Task DumpAsync_WhenCancellationRequested_ShouldStopGracefully()
    {
        ...
        // Act
        Func<Task> act = async () => await service.DumpAsync(profiles, progress, null, null, cts.Token);
    ...
    public async Task DumpAsync_WhenMemoryDumpFails_ShouldThrowException()
    {
        ...
        plcClient.GetBootloaderVersionAsync(Arg.Any<CancellationToken>()).Returns("1.0.0");
        plcClient.InstallStagerAsync(Arg.Any<byte[]>(), Arg.Any<CancellationToken>()).Returns(Task.CompletedTask);
        plcClient.InvokeDumperStreamAsync(Arg.Any<uint>(), Arg.Any<uint>(), Arg.Any<Func<ReadOnlyMemory<byte>, ValueTask>>(), Arg.Any<IProgress<long>>(), Arg.Any<CancellationToken>(), Arg.Any<bool>(), Arg.Any<Microsoft.Extensions.Logging.ILogger?>())
